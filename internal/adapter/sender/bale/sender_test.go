...
// bot stands in for the Bale API: it records what it was asked and answers with
// whatever the test wants.
func bot(t *testing.T, status int, answer string) (*bale.Sender, *request) {
	t.Helper()
	return botWithConfig(t, status, answer, nil)
}

func botWithConfig(t *testing.T, status int, answer string, config []byte) (*bale.Sender, *request) {
	t.Helper()
	...
	s, err := bale.New(server.Client(), token, config)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func msg(title, body string) shared.Message {
	return shared.Message{
		Recipient: shared.Recipient{Channel: shared.ChannelBale, Address: "-1001234"},
		Title:     title,
		Body:      body,
	}
}
...
func TestASentMessageComesBackWithItsID(t *testing.T) {
	...
	if id != "4242" {
		t.Errorf("id = %q, want the message_id bale gave", id)
	}
...
// Bale has one text field, so a title and a body have to become one string.
...
// A message bale will refuse for its length is refused here, before the
...
func TestAProviderWeCannotReachIsWorthAnotherTry(t *testing.T) {
	...
	s, err := bale.New(http.DefaultClient, token, nil)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func TestASenderNeedsATokenAndReadableSettings(t *testing.T) {
	if _, err := bale.New(http.DefaultClient, "", nil); !errs.IsType(err, errs.ErrInvalidInput) {
		t.Errorf("New with no token = %v, want invalid input", err)
	}
	if _, err := bale.New(http.DefaultClient, token, []byte("parse_mode=HTML")); !errs.IsType(err, errs.ErrInvalidInput) {
		t.Errorf("New with unreadable settings = %v, want invalid input", err)
	}
	if _, err := bale.New(nil, token, nil); err == nil {
...
func TestATokenCannotChooseTheEndpoint(t *testing.T) {
	for _, bad := range []string{"1:a/../../x", "1:a b", "1:a\nHost: evil", "1:a?x=1", "1:a#f"} {
		if _, err := bale.New(http.DefaultClient, bad, nil); !errs.IsType(err, errs.ErrInvalidInput) {
			t.Errorf("New(%q) = %v, want invalid input", bad, err)
		}
	}
}

// The API puts the credential in the path, so a url from this sender is the
