// Package bale sends through Bale's bot API, and is the only place that knows
// anything about it: its address, its json, its status codes and what each of
// them means for a retry.
//
// It reads like the telegram package because the two APIs are shaped alike
// today, and it is its own package because they are two providers. Which
// message length is accepted, which parse modes exist, and whether a 429 states
// its own wait are each of them theirs to change alone.
package bale
...
type Config struct {
	...
	// it and the body travels as written: a source that asks for markup owns
	// escaping it, because escaping on their behalf would break the markup they
	// meant.
...
func New(client *http.Client, token string, rawConfig []byte) (*Sender, error) {
	if client == nil {
		return nil, errs.InternalErr("bale sender has no http client")
	}
	if token == "" {
		...
		// cannot exist.
		return nil, errs.InvalidInputErr("no bale token for this identity")
	}
	if !usableToken(token) {
		// Never echoed, whatever it turned out to be.
		return nil, errs.InvalidInputErr("bale token has the wrong format")
	}

	var config Config
	if len(rawConfig) > 0 {
		if err := json.Unmarshal(rawConfig, &config); err != nil {
			return nil, errs.InvalidInputErr("bale settings are not valid json").WithErr(err)
		}
	}
...
func (s *Sender) Channel() shared.Channel { return shared.ChannelBale }

// Send posts one message and returns the id the API gave it.
//
...
func (s *Sender) Send(ctx context.Context, m shared.Message) (string, error) {
	...
	if utf8.RuneCountInString(text) > maxTextLen {
		return "", refused(fmt.Sprintf("message is longer than bale accepts (max %d)", maxTextLen))
	}
...
// logged: the credential goes in the path rather than in a header, so a url
// from this sender IS the secret.
